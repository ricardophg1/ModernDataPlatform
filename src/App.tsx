...
const PREMIUM_SECTIONS = ['models', 'pipelines', 'jobs'];

function App() {
  const [activeSection, setActiveSection] = useState('workspace');
  const [subscriptionPlan] = useState<'free' | 'team'>('free'); // 'free' or 'team'

  const renderSection = () => {
    if (PREMIUM_SECTIONS.includes(activeSection) && subscriptionPlan === 'free') {
      return <UpgradeModal onNavigate={setActiveSection} />;
    }
